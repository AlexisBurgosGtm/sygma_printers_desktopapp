...
Module GeneralModule
    ...
    Public Function establecer_conexion() As Boolean
        ...
        Using cn As New SqlConnection(strSqlConectionString)
            Try
                ...
                Dim sql As String = "
				    ...
				    HAVING (TIPODOCUMENTOS.TIPODOC IN ('FAC', 'FCP', 'FEC', 'FEF', 'FES', 'FPC')) 
    ...
    Public Function rptEmbarqueBoletas(ByVal codembarque As String) As Boolean
        ...
        Using cnh As New SqlConnection(strSqlConectionString)
            Try
                ...
                Dim qryxx As String = "SELECT DOCUMENTOS.FECHA, 
	DOCUMENTOS.CODDOC, 
	DOCUMENTOS.CORRELATIVO, 
	CLIENTES.NIT, 
	CONCAT(ISNULL(CLIENTES.TIPONEGOCIO, 'TIENDA'), ' ', ISNULL(CLIENTES.NEGOCIO, 'SN'), '-', DOCUMENTOS.DOC_NOMCLIE) AS NOMCLIENTE, 
	CASE WHEN DOCUMENTOS.DOC_DIRCLIE = 'CIUDAD' THEN CLIENTES.DIRECCION ELSE DOCUMENTOS.DOC_DIRCLIE END AS DIRCLIENTE, ISNULL(CLIENTES.EMAIL, 'SN') AS EMAIL, 
                  DOCUMENTOS.TOTALPRECIO AS TOTALVENTA, ISNULL(DOCUMENTOS.OBS, '') AS OBS, 
				  DOCUMENTOS.CODEMBARQUE, ISNULL(CLIENTES.LATITUD, '0') AS LAT, 
				  ISNULL(CLIENTES.LONGITUD, '0') AS LONG, 
                  ISNULL(EMPLEADOS.NOMEMPLEADO, 'SN') AS DESREP, 
				  ISNULL(EMPLEADOS.NOMEMPLEADO, 'SN') AS NOMVEN, 
				  DOCPRODUCTOS.CODPROD, ISNULL(DOCPRODUCTOS.DESPROD, 'SN') AS DESPROD, DOCPRODUCTOS.CODMEDIDA, 
                  DOCPRODUCTOS.CANTIDAD, DOCPRODUCTOS.PRECIO, DOCPRODUCTOS.TOTALPRECIO, PRODUCTOS.UXC, 
				  DOCPRODUCTOS.TOTALUNIDADES, ISNULL(CLIENTES.CODCLIENTE, '0') AS CODCLIENTE, 
                  ISNULL(CLIENTES.REFERENCIA, '') AS REFERENCIA, ISNULL(CLIENTES.NEGOCIO, 'SN') AS NEGOCIO, 
				  EMPLEADOS.TELEFONO AS TELVENDEDOR, ISNULL(DOCUMENTOS.CONCRE, 'CON') AS TIPOPAGO, 
				  ISNULL(CLIENTES.TELEFONO, 'SN') AS TELCLIE, EMPRESAS.NOMBRE AS EMPRESA
FROM     CLIENTES RIGHT OUTER JOIN
                  EMPRESAS RIGHT OUTER JOIN
                  DOCUMENTOS ON EMPRESAS.EMPNIT = DOCUMENTOS.EMPNIT ON CLIENTES.CODCLIENTE = DOCUMENTOS.CODCLIENTE AND CLIENTES.EMPNIT = DOCUMENTOS.EMPNIT LEFT OUTER JOIN
                  EMPLEADOS RIGHT OUTER JOIN
                  EMBARQUES ON EMPLEADOS.CODEMPLEADO = EMBARQUES.CODEMPLEADO AND EMPLEADOS.EMPNIT = EMBARQUES.EMPNIT ON DOCUMENTOS.CODEMBARQUE = EMBARQUES.CODEMBARQUE AND 
                  DOCUMENTOS.EMPNIT = EMBARQUES.EMPNIT LEFT OUTER JOIN
                  PRODUCTOS RIGHT OUTER JOIN
                  DOCPRODUCTOS ON PRODUCTOS.CODPROD = DOCPRODUCTOS.CODPROD ON DOCUMENTOS.CORRELATIVO = DOCPRODUCTOS.CORRELATIVO AND DOCUMENTOS.CODDOC = DOCPRODUCTOS.CODDOC AND 
...
WHERE  (TIPODOCUMENTOS.TIPODOC IN ('FAC', 'FCP', 'FEC', 'FEF', 'FES', 'FPC')) AND (DOCUMENTOS.STATUS <> 'A') AND (DOCUMENTOS.EMPNIT = @E) AND (DOCUMENTOS.CODEMBARQUE = @C)"
                ...
                Dim qry As String = "
                SELECT DOCUMENTOS.FECHA, DOCUMENTOS.CODDOC, DOCUMENTOS.CORRELATIVO, CLIENTES.NIT, 
                        CONCAT(ISNULL(CLIENTES.TIPONEGOCIO, 'TIENDA'), ' ', ISNULL(CLIENTES.NEGOCIO, 'SN'), '-', DOCUMENTOS.DOC_NOMCLIE) AS NOMCLIENTE, 
                            CASE WHEN DOCUMENTOS.DOC_DIRCLIE = 'CIUDAD' THEN CLIENTES.DIRECCION ELSE DOCUMENTOS.DOC_DIRCLIE END AS DIRCLIENTE, ISNULL(CLIENTES.EMAIL, 'SN') AS EMAIL, 
                                  DOCUMENTOS.TOTALPRECIO AS TOTALVENTA, ISNULL(DOCUMENTOS.OBS, '') AS OBS, DOCUMENTOS.CODEMBARQUE, ISNULL(CLIENTES.LATITUD, '0') AS LAT, ISNULL(CLIENTES.LONGITUD, '0') AS LONG, 
                                  ISNULL(EMPLEADOS_1.NOMEMPLEADO, 'SN') AS DESREP, 
                                    ISNULL(EMPLEADOS_1.NOMEMPLEADO, 'SN') AS NOMVEN, 
                                    DOCPRODUCTOS.CODPROD, ISNULL(DOCPRODUCTOS.DESPROD, 'SN') AS DESPROD, 
                                  DOCPRODUCTOS.CODMEDIDA, DOCPRODUCTOS.CANTIDAD, DOCPRODUCTOS.PRECIO, DOCPRODUCTOS.TOTALPRECIO, PRODUCTOS.UXC, DOCPRODUCTOS.TOTALUNIDADES, ISNULL(CLIENTES.CODCLIENTE, '0') 
                                  AS CODCLIENTE, ISNULL(CLIENTES.REFERENCIA, '') AS REFERENCIA, ISNULL(CLIENTES.NEGOCIO, 'SN') AS NEGOCIO, 
                                  EMPLEADOS.TELEFONO AS TELVENDEDOR, 
                                ISNULL(DOCUMENTOS.CONCRE, 'CON') AS TIPOPAGO, 
                                  ISNULL(CLIENTES.TELEFONO, 'SN') AS TELCLIE, EMPRESAS.NOMBRE AS EMPRESA, EMPLEADOS.NOMEMPLEADO
                FROM     EMPRESAS RIGHT OUTER JOIN
                                  EMPLEADOS RIGHT OUTER JOIN
                                  DOCUMENTOS ON EMPLEADOS.CODEMPLEADO = DOCUMENTOS.CODEMP ON EMPRESAS.EMPNIT = DOCUMENTOS.EMPNIT LEFT OUTER JOIN
                                  CLIENTES ON DOCUMENTOS.CODCLIENTE = CLIENTES.CODCLIENTE AND DOCUMENTOS.EMPNIT = CLIENTES.EMPNIT LEFT OUTER JOIN
                                  EMPLEADOS AS EMPLEADOS_1 RIGHT OUTER JOIN
                                  EMBARQUES ON EMPLEADOS_1.CODEMPLEADO = EMBARQUES.CODEMPLEADO AND EMPLEADOS_1.EMPNIT = EMBARQUES.EMPNIT ON DOCUMENTOS.CODEMBARQUE = EMBARQUES.CODEMBARQUE AND 
                                  DOCUMENTOS.EMPNIT = EMBARQUES.EMPNIT LEFT OUTER JOIN
                                  PRODUCTOS RIGHT OUTER JOIN
                                  DOCPRODUCTOS ON PRODUCTOS.CODPROD = DOCPRODUCTOS.CODPROD ON DOCUMENTOS.CORRELATIVO = DOCPRODUCTOS.CORRELATIVO AND DOCUMENTOS.CODDOC = DOCPRODUCTOS.CODDOC AND 
                                  DOCUMENTOS.MES = DOCPRODUCTOS.MES AND DOCUMENTOS.EMPNIT = DOCPRODUCTOS.EMPNIT AND DOCUMENTOS.ANIO = DOCPRODUCTOS.ANIO LEFT OUTER JOIN
