            ...
            Try
                ...
                sqlx = "
                    SELECT DOCPRODUCTOS.CODPROD, DOCPRODUCTOS.DESPROD, SUM(DOCPRODUCTOS.TOTALUNIDADES) AS TOTALUNIDADES, SUM(DOCPRODUCTOS.TOTALCOSTO) AS TOTALCOSTO, SUM(DOCPRODUCTOS.TOTALPRECIO) 
                                      AS TOTALPRECIO, DOCUMENTOS.CODEMBARQUE, PRODUCTOS.UXC, EMPLEADOS.NOMEMPLEADO AS REPARTIDOR, EMBARQUES.DESCRIPCION AS DESEMBARQUE, SUM(DOCPRODUCTOS.TOTALUNIDADES) 
                                      / PRODUCTOS.UXC AS FACTOR
                    FROM     EMBARQUES RIGHT OUTER JOIN
                                      DOCUMENTOS LEFT OUTER JOIN
                                      EMPLEADOS ON DOCUMENTOS.CODEMP = EMPLEADOS.CODEMPLEADO AND DOCUMENTOS.EMPNIT = EMPLEADOS.EMPNIT ON EMBARQUES.EMPNIT = DOCUMENTOS.EMPNIT AND 
                                      EMBARQUES.CODEMBARQUE = DOCUMENTOS.CODEMBARQUE LEFT OUTER JOIN
                    ...
                    GROUP BY DOCPRODUCTOS.CODPROD, DOCPRODUCTOS.DESPROD, DOCUMENTOS.CODEMBARQUE, DOCUMENTOS.EMPNIT, PRODUCTOS.UXC, TIPODOCUMENTOS.TIPODOC, EMPLEADOS.NOMEMPLEADO, DOCUMENTOS.STATUS, 
                                      EMBARQUES.DESCRIPCION
                    HAVING (TIPODOCUMENTOS.TIPODOC IN ('FAC', 'FCP', 'FEC', 'FEF', 'FES', 'FPC')) AND (DOCUMENTOS.STATUS <> 'A') AND (DOCUMENTOS.EMPNIT = @E) AND (DOCUMENTOS.CODEMBARQUE = @C)
                    ORDER BY DOCPRODUCTOS.CODPROD"
    ...
    Public Function rptEmbarqueBoletas(ByVal codembarque As String) As Boolean
        ...
        Using cnh As New SqlConnection(strSqlConectionString)
            Try
                ...
                Dim qry As String = ""
                qry = "SELECT DOCUMENTOS.FECHA, DOCUMENTOS.CODDOC, DOCUMENTOS.CORRELATIVO, CLIENTES.NIT, 
                    CONCAT(ISNULL(CLIENTES.NEGOCIO, 'SN'), '-',DOCUMENTOS.DOC_NOMCLIE,) AS NOMCLIENTE, 
                  CASE WHEN DOCUMENTOS.DOC_DIRCLIE = 'CIUDAD' THEN CLIENTES.DIRECCION ELSE DOCUMENTOS.DOC_DIRCLIE END AS DIRCLIENTE, 
                    ISNULL(CLIENTES.EMAIL, 'SN') AS EMAIL, 
                    DOCUMENTOS.TOTALPRECIO AS TOTALVENTA, 
                  ISNULL(DOCUMENTOS.OBS, '') AS OBS, 
                    DOCUMENTOS.CODEMBARQUE, 
                ...
                DOCPRODUCTOS.CODPROD, ISNULL(DOCPRODUCTOS.DESPROD, 'SN') AS DESPROD, DOCPRODUCTOS.CODMEDIDA, DOCPRODUCTOS.CANTIDAD, 
                  DOCPRODUCTOS.PRECIO, DOCPRODUCTOS.TOTALPRECIO, PRODUCTOS.UXC, DOCPRODUCTOS.TOTALUNIDADES, ISNULL(CLIENTES.CODCLIENTE, '0') AS CODCLIENTE, 
                    ISNULL(CLIENTES.REFERENCIA, '') AS REFERENCIA, 
                  ISNULL(CLIENTES.NEGOCIO, 'SN') AS NEGOCIO, 
                    EMPLEADOS.TELEFONO AS TELVENDEDOR, 
                    ISNULL(DOCUMENTOS.CONCRE, 'CON') AS TIPOPAGO, 
                    ISNULL(CLIENTES.TELEFONO, 'SN') AS TELCLIE
        ...
        Using cnh As New SqlConnection(strSqlConectionString)
            Try
                ...
                qryx = "SELECT DOCUMENTOS.FECHA, 
                            ...
                            ISNULL(DOCPRODUCTOS.DESPROD, 'SN') AS DESPROD, 
                            ...
                            ISNULL(CLIENTES.CODCLIENTE, '0') AS CODCLIENTE, 
                            ISNULL(CLIENTES.REFERENCIA, '') AS REFERENCIA, 
                            ISNULL(CLIENTES.NEGOCIO, 'SN') AS NEGOCIO, 
                            EMPLEADOS.TELEFONO AS TELVENDEDOR, 
                            ISNULL(DOCUMENTOS.CONCRE, 'CON') AS TIPOPAGO, 
                ...
                Dim qry As String = "
                    SELECT DOCUMENTOS.FECHA, DOCUMENTOS.CODDOC, DOCUMENTOS.CORRELATIVO, CLIENTES.NIT, DOCUMENTOS.DOC_NOMCLIE AS NOMCLIENTE, 
                    ...
                    FROM     PRODUCTOS RIGHT OUTER JOIN
                                      DOCPRODUCTOS ON PRODUCTOS.CODPROD = DOCPRODUCTOS.CODPROD RIGHT OUTER JOIN
                                      EMPLEADOS RIGHT OUTER JOIN
                                      DOCUMENTOS ON EMPLEADOS.CODEMPLEADO = DOCUMENTOS.CODEMP AND EMPLEADOS.EMPNIT = DOCUMENTOS.EMPNIT LEFT OUTER JOIN
                                      CLIENTES ON DOCUMENTOS.CODCLIENTE = CLIENTES.CODCLIENTE AND DOCUMENTOS.EMPNIT = CLIENTES.EMPNIT ON DOCPRODUCTOS.CORRELATIVO = DOCUMENTOS.CORRELATIVO AND 
                                      DOCPRODUCTOS.CODDOC = DOCUMENTOS.CODDOC AND DOCPRODUCTOS.MES = DOCUMENTOS.MES AND DOCPRODUCTOS.EMPNIT = DOCUMENTOS.EMPNIT AND DOCPRODUCTOS.ANIO = DOCUMENTOS.ANIO LEFT OUTER JOIN
                                      TIPODOCUMENTOS ON DOCUMENTOS.CODDOC = TIPODOCUMENTOS.CODDOC AND DOCUMENTOS.EMPNIT = TIPODOCUMENTOS.EMPNIT LEFT OUTER JOIN
                                      EMBARQUES ON DOCUMENTOS.CODEMBARQUE = EMBARQUES.CODEMBARQUE AND DOCUMENTOS.EMPNIT = EMBARQUES.EMPNIT
                    WHERE  (TIPODOCUMENTOS.TIPODOC IN ('FAC', 'FCP', 'FEC', 'FEF', 'FES', 'FPC')) AND (DOCUMENTOS.STATUS <> 'A') AND (DOCUMENTOS.EMPNIT = @E) AND (DOCUMENTOS.CODEMBARQUE = @C)
                "
